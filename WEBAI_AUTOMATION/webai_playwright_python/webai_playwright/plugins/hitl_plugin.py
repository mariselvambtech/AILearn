...
class HITLPlugin:
    ...
    async def _inject_and_await_click(self, page: Page, timeout_sec: float = 30.0) -> Dict[str, Any]:
        """Injects JS click listener into DOM and awaits click via Promise to avoid navigation traps."""
        js_code = """() => {
            return new Promise((resolve) => {
                const handler = (e) => {
                    // 1. Stop the browser from navigating away
                    e.preventDefault();
                    // 2. Stop other website scripts from handling the click
                    e.stopPropagation();
        ...
        try:
            # Await the Promise directly. Playwright will pause until resolve() is called in the browser.
            click_data = await asyncio.wait_for(page.evaluate(js_code), timeout=timeout_sec)
            return click_data
            
        except asyncio.TimeoutError:
            print(" ⚠️ [HITLPlugin] Timeout waiting for manual click.")
            return {
                "x": 0, "y": 0,
                "targetTag": "UNKNOWN", "targetId": "",
                "textContent": "Timeout waiting for manual click",
            }
        except Exception as e:
            print(f" ⚠️ [HITLPlugin] Failed to inject/await JS click listener: {e}")
            return {
                "x": 0, "y": 0,
