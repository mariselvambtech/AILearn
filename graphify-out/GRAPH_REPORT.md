...
## Corpus Check
- 288 files · ~281,993 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1523 nodes · 2307 edges · 120 communities (107 shown, 13 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 40 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `db8b8692`
...
5. `Decision Log — WebAI Platform` - 25 edges
6. `get_cdp()` - 21 edges
7. `Recent Work Completed` - 20 edges
8. `SkillExecutor` - 20 edges
9. `click()` - 19 edges
10. `HITLPlugin` - 18 edges
...
- `DummySegment` --uses--> `AudioAligner`  [INFERRED]
  WEBAI_AUTOMATION/webai_playwright_python/test_audio_transcriber.py → WEBAI_AUTOMATION/webai_playwright_python/webai_playwright/audio_aligner.py
- `DummySegment` --uses--> `BaseTranscriber`  [INFERRED]
  WEBAI_AUTOMATION/webai_playwright_python/test_audio_transcriber.py → WEBAI_AUTOMATION/webai_playwright_python/webai_playwright/audio_transcriber.py
- `DummySegment` --uses--> `SenseVoiceTranscriber`  [INFERRED]
  WEBAI_AUTOMATION/webai_playwright_python/test_audio_transcriber.py → WEBAI_AUTOMATION/webai_playwright_python/webai_playwright/audio_transcriber.py
...
Cohesion: 0.15
Nodes (13): Current Session (2026-09-20), Phase 17: Distil-Whisper Transcription & Vocabulary Priming Upgrade (`audio_aligner.py`) ✅, Phase 18: Semantic Verification & Conditional Assertions (Decision 14, Rule 7 TDVC) ✅, Phase 19: New Tab Automatic Context Switching (`skill_executor.py`) (Rule 7 TDVC) ✅, Phase 20: Semantic Verification HITL Interception & Auto-Resumption (Rule 7 TDVC, Rule 9) ✅, Phase 21: Automation-to-Skill Mapping & Nested Dashboard Rendering (Rule 7 TDVC, Rule 3) ✅, Phase 22: Decoupled Skill Synthesis Subprocess Execution (`dash_synthesize.py`) (Rule 3, Rule 7 TDVC) ✅, Phase 23: Windows Console UTF-8 & Global `safe_print` Wrapper (`skill_executor.py`, `dashboard_server.py`) (Rule 3) ✅ (+5 more)

### Community 2 - "Open Questions / Decisions Pending"
Cohesion: 0.17
Nodes (10): DataExtractionPlugin, Any, Plugin that listens to data extraction events ('extract' and 'extract_table'), Attach plugin to WebRecorder instance by subscribing to event bus., Handle standard text/attribute extraction event., Handle table extraction event., Save single extraction to files immediately., Save extraction to Excel file. (+2 more)
...
### Community 21 - "Page"
Cohesion: 0.18
Nodes (8): TDVC Test Harness for AudioAligner Temporal Alignment Logic. Verifies time windo, test_audio_alignment_math(), TDVC Test Harness for AudioAligner VAD & Anti-Hallucination Configuration. Verif, Assert that AudioAligner.transcribe_audio calls model.transcribe with:     - vad, Assert that AudioAligner defaults to 'base.en' and falls back gracefully to 'bas, test_audio_vad_transcribe_kwargs(), test_model_upgrade_and_fallback(), Audio Aligner Utility for WebAI Playwright Recorder.  Transcribes recorded sessi
...
### Community 26 - "Any"
Cohesion: 0.12
Nodes (25): _buffer_log(), delete_automation(), delete_skill_endpoint(), _flush_orchestration_logs(), get_automation(), get_execution_logs(), list_automations(), list_executions() (+17 more)

### Community 27 - "run_automation_endpoint"
Cohesion: 0.14
Nodes (15): BaseModel, login(), LoginPayload, Payload for starting a new interactive recording session from the dashboard., Result of a successfully dispatched automation run., Choose the Python interpreter for the playback subprocess.      Prefers the Pl, Proxy login to the API server and return the user's API key.      The front-en, Trigger browser playback of a database-backed automation.      Replaces the in (+7 more)

### Community 28 - "dashboard_server.py"
Cohesion: 0.13
Nodes (18): dashboard_health(), execute_skill_endpoint(), list_skills(), _probe_http(), _probe_tcp(), _probe_ws(), Path, WebAI Dashboard Server — web UI + orchestration API.  This FastAPI server is t (+10 more)

### Community 29 - "MonkeyPatch"
Cohesion: 0.09
Nodes (14): MonkeyPatch, FakeResponse, Any, Health endpoint always reports dashboard status plus dependency probes., Proxy endpoints forward auth headers and surface upstream errors., Import endpoint rejects malformed uploads before touching the API server., Run endpoint orchestration (subprocess spawn stubbed out)., End-to-end proxy chain against a real API server (no browser launched). (+6 more)
...
Cohesion: 0.20
Nodes (10): Response, UploadFile, import_automation(), Extract an error detail payload from an upstream API response.      Args:, Proxy user registration to the API server., Registration details forwarded to the API server's /auth/register endpoint., Import a recorded_steps.json file as a new database automation.      Replaces, register() (+2 more)
...
Cohesion: 0.06
Nodes (34): Any, action_listener_loop(), load_local_skills(), main(), End-to-End Hybrid Test Orchestrator for WebAI Platform.  1. Accepts hardcoded pr, Discovers and loads synthesized skills from JSON files in the workspace., Listens for incoming AI action commands (command-request) from local_webai_serve, TDVC Test Suite for Phase 6: Semantic Intent Router & Agentic Handoff Engine (Ru (+26 more)
...
Cohesion: 0.20
Nodes (9): Context, Decision 14: Semantic Verification & Conditional Assertions, Decision 21: Jira Integration Approach (Pending), Decision 22: Variable Persistence Model (Pending), Decision, Decision Log — WebAI Platform, Future Decisions Pending, Impact (+1 more)
...
Cohesion: 0.06
Nodes (32): Page, Step, create_mock_page(), Any, TDVC Test Harness for HITLPlugin & Event Bus Interception (Phase 10 & 11). Verif, Create a mocked Playwright Page object simulating Observer Mode resolution., Assert HITLPlugin packages observer resolution data and transcribed voice into p, Assert HITLPlugin handles hardware/audio transcription failures gracefully witho (+24 more)

### Community 66 - "Step"
Cohesion: 0.07
Nodes (28): Exception, TDVC Test Suite for Automation-to-Skill Mapping: 1. SkillSynthesizer.synthesize(, Validates POST /api/automations/{automation_id}/synthesize endpoint workflow., Validates that synthesize() adds source_automation_id at root level of skill dic, Validates that save_skill writes source_automation_id to disk in both slug and m, Validates that list_skills includes source_automation_id in the response payload, test_list_skills_extracts_source_automation_id(), test_save_skill_persists_source_automation_id() (+20 more)
...
Cohesion: 0.16
Nodes (14): dummy_wav(), Unit and integration tests for Audio Transcriber Strategy Pattern & Factory. Tes, Verify SenseVoiceTranscriber raises ImportError with pip instructions when funas, Creates a valid dummy WAV file >= 100 bytes., Verify get_transcriber factory returns expected strategy based on argument and e, Verify SenseVoiceTranscriber generation, postprocessing, and 0..999999ms segment, test_factory_model_selection(), test_sense_voice_missing_dependency_warning() (+6 more)
...
### Community 81 - "Any"
Cohesion: 0.16
Nodes (11): ask_yes_no(), main(), Verify AudioAligner integrates with transcriber strategy and aligns steps., test_audio_aligner_integration(), AudioAligner, Any, Utility class for transcribing audio and aligning voice context with recorded st, Forwarding method to underlying strategy model for backwards compatibility. (+3 more)
...
Cohesion: 0.17
Nodes (7): Any, Lazy initialization of FunASR SenseVoice model with graceful dependency warning., Transcribe audio using SenseVoice and clean rich tags with postprocess_utils., Returns a single segment spanning the entire audio session (0..999999 ms), Transcribe the audio file into timestamped segments.          Args:, Transcribe WAV audio into joined text., Transcribe WAV audio into timestamped segments in milliseconds.

### Community 89 - ".json"
Cohesion: 0.25
Nodes (6): DummySegment, Verify FasterWhisperTranscriber segments and text transcription., test_faster_whisper_transcriber(), FasterWhisperTranscriber, faster-whisper transcription strategy utilizing hardware auto-detection (CUDA/CP, Lazy initialization of faster-whisper model.
...
Cohesion: 0.16
Nodes (12): TDVC Test Harness for AudioCapturePlugin & WebRecorder Synchronization. Verifies, test_audio_plugin_synchronization(), TDVC Test Harness for SkillSynthesizer. Verifies Ollama synthesis, JSON parsing,, test_skill_synthesis(), fetch_steps_from_api(), main(), main(), Audio Capture Plugin for WebAI Playwright Recorder.  Provides background audio r (+4 more)
...
Cohesion: 0.33
Nodes (4): Initialize AudioAligner with an explicit transcriber strategy, or fallback to AC, BaseTranscriber, Abstract Base Class for voice model transcription strategies., Transcribe the audio file directly into a clean, plain text string.          Arg
...
### Community 104 - "MockLocator"
Cohesion: 0.50
Nodes (4): Context, Decision, Decision: Refactoring `recorder.py` to Event Bus & Plugin Isolation, Impact

### Community 105 - "tests/test_dashboard_api.py"
Cohesion: 0.50
Nodes (4): Context, Decision 20: Transcriber Strategy Pattern & SenseVoice Support, Decision, Impact

### Community 106 - "validate_steps_payload"
Cohesion: 0.14
Nodes (11): _api_server_online(), QA suite for the WebAI Dashboard Server (webai_dashboard).  Covers two layers:, generated_task.txt content generation for guided playback., Return True when a real API server answers on WEBAI_API_URL., Step-payload validation for the import endpoint., TestBuildTaskText, TestValidateStepsPayload, build_task_text() (+3 more)
...
Cohesion: 0.22
Nodes (9): Active Context  WebAI Platform, Dashboard Enhancements & Performance Optimization, Phase 15: Kimi K3 Provider Integration & Multimodal Visual-DOM Planning (Rule 7 TDVC) ✅, Phase 16: Multi-Skill Library Integration & Catalog Scanner (Rule 7 TDVC) ✅, Plugin Architecture Refactoring: `recorder.py` & `DataExtractionPlugin` ✅, Previous Session (2026-07-18), Previous Session (2026-08-03), Previous Session (2026-09-18) (+1 more)
...
- **Why does `SkillSynthesizer` connect `Step` to `Any`, `test_dashboard_api.py`, `validate_steps_payload`, `skill_synthesizer.py`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `AudioAligner` connect `Any` to `.json`, `MockPage`, `Page`, `execute_skill_endpoint`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `Step` connect `skill_synthesizer.py` to `Open Questions / Decisions Pending`, `HITLPlugin`, `validate_steps_payload`, `AudioCapturePlugin`, `dash_synthesize.py`, `Page`, `skill_executor.py`, `Page`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **What connects `Phase 28: Transcriber Strategy Pattern & SenseVoice Support (Decision 20, Rule 7 TDVC) ✅`, `Phase 27: Dashboard Skill Management (Delete Functionality) (Decision 19, Rule 7 TDVC) ✅`, `Phase 26: Skill Synthesis (Auto-Saving Successful Agentic Workflows) (Decision 18, Rule 7 TDVC) ✅` to the rest of the system?**
